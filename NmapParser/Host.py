...
class Host:
    ...
    def __init__( self, HostNode ):
        ...
        self.mac = '';
        self.hostname = '';
        for elem in HostNode.getElementsByTagName('address'):
            if elem.getAttribute('addrtype') == 'ipv4':
                #self.ip = HostNode.getElementsByTagName('address')[0].getAttribute('addr');
                self.ip = elem.getAttribute('addr');
            elif elem.getAttribute('addrtype') == 'mac':
                #self.mac = str(HostNode.getElementsByTagName('address')[1].getAttribute('addr'));
                self.mac = str(elem.getAttribute('addr')) + ' - ' + str(elem.getAttribute('vendor'));
        for host_name in HostNode.getElementsByTagName('hostnames'):
            if host_name.getElementsByTagName('hostname'):
                self.hostname = str(host_name.getElementsByTagName('hostname')[0].getAttribute('name'));
        #self.ip = HostNode.getElementsByTagName('address')[0].getAttribute('addr');
